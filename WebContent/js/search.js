...
function request_one_page() {
	...
	$.ajax({
		type: 'get',
		url: ("/group5/ControllerHelp?command=search&category=" + category + "&search=" + search + "&resultStyle=" + resultStyle + "&total=" + total + "&page=" + page),
		dataType: 'json',
		success: function(dataList) {
			for (let data of dataList) {
				...
				articleLink.setAttribute("href", `/group5/	help/search/faqDetail/?command=viewFaq&fno=${data.fno}`);
