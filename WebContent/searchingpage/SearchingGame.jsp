...
<title>찾아보기</title>
<meta charset="UTF-8">
<link rel="stylesheet" href="/group5/css/find.css" />
...
<script>
	function showHiddenEvent() {
		...
		if (hiddenEvent.classList.contains("show")) {
			...
			img.src = "/group5/images/32196.png";
		} else {
			hiddenEvent.classList.add("show");
			img.src = "/group5/images/32197.png";
		}
	}
...
<style>
#hover-img2{
	 transition: filter 0.3s ease; /* 호버 효과에 대한 부드러운 전환을 추가합니다. */
}
#hover-img2:hover{
	background:white;
...
">
					...
					<div class="subprice-item4">
						...
						<span style="color: white"><div id="post_searchGlass"
								style="position: relative;top: 257px;right: 0px;width: 24px;height: 40px;/* bottom: -235px; */left: 1046px;border-radius: 10px 0 0 10px;"></div>
							<input type="text"  id="searchInput name="Commu_Search"
							placeholder="키워드" spellcheck="false" autocomplete="off"
							class="search-input"
							style="background: rgb(32, 32, 32);position: relative;bottom: -195px;left: 1062px;width: 215px;height: 42px;z-index: 9000;color: white;border-radius: 10px;">
							<img src="/group5/images/1111.png" alt="돋보기"
							style="position: relative;top: 200px;left: 810px;width: 20px;height: 20px;z-index: 9000;"></span>
					</div>
					<div class="my"style="
    position: absolute;
">
	...
	">
						...
						<button id="eventButton" class="button eventButton"
							...
							이벤트<img id="eventImage" src="/group5/images/32196.png" alt="이벤트 보기 1"
						...
						<button class="button eventButton" onclick="showHiddenPrice()"
							style="border-bottom: 1px solid gray;">
							가격 <img id="priceImage" src="/group5/images/32196.png" alt="이벤트 보기 1"
						...
						<button class="button eventButton" onclick="showHiddenGenre()"
							style="border-bottom: 1px solid gray;">
							장르 <img id="genreImage" src="/group5/images/32196.png" alt="이벤트 보기 1"
								style="width: 15px; height: 15px; position: relative; left: 180px; top: 0px;">
						</button>
        ...
        <button class="button eventButton"  onclick="getGames('RTS', this)">&nbsp;&nbsp;RTS</button><br>
						...
						<button class="button eventButton" onclick="showHiddenPlatform()"
							style="border-bottom: 1px solid gray;">
							플랫폼 <img id="platformImage" src="/group5/images/32196.png" alt="이벤트 보기 1"
...
<script>
	function qwer(count) {
		  var linkUrl = "http://localhost:9090:8090/epicgames/gamepage.jsp";
...
<table>
    <% Connection conn = getConnection();
       ...
       while (rs.next()) {
           ...
           %><td id=<%=count %> onclick="qwer(<%=count%>)"><div class="c" ><img src=<%= rs.getString("thumnail_img") %> id="hover-img2" style="width:220px;height:290px;border-radius:10px;"></div><br>
               <div style="font-size:13px;color:gray;">기본 게임</div><br>
               <div class="d"> <%= rs.getString("title") %></div>
             <%if(rs.getInt("ori_prc")==0){ %> 무료 
             <%}else if(rs.getInt("ori_prc")>0&&(int)(rs.getDouble("dc_rate")*100)!=0){ %><span style="
    border: 1px solid blue;
    background: blue;
    border-radius: 5px;
        font-size: 16px;
    
"><%=(int)(rs.getDouble("dc_rate")*100)%>%</span>&emsp;<span style="color:gray;font-size: 18px;"><strike>₩<%= rs.getString("ori_prc") %></strike></span>&emsp;<span style="font-size: 18px;">₩<%= (int)(-(rs.getInt("ori_prc"))*(rs.getDouble("dc_rate"))) %></span>
             <%}else if(rs.getInt("ori_prc")>0&&(int)(rs.getDouble("dc_rate")*100)==0){ %><span style="font-size: 18px;">₩<%= rs.getString("ori_prc") %></span>
             <%}else{ %> 미출시   <%} %><br></td><%
           count++;
           if (count % 5 == 0) {
               %></tr><%
