...
<%@ page import="kotlinx.datetime.Clock.System" %>
<%
	...
	String confirmNum = (String)request.getAttribute("confirmNum");
%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>email OTP for YG games</title>
<link href="css/emailStyle.css" rel="stylesheet" type="text/css" />
...
<body>
	<main>
		<div id="outerbox">
			...
			<h6>이메일을 확인해 주세요</h6>
				<p>계정설정을 완료하려면 <b><% out.print(email); %></b>을(를) 인증해야 합니다. 이메일을 확인한 뒤 다음 아래에 보안 코드를 입력하세요.</p>
				<div id="innercenter">
					<div id="writenum">
				...
				<div id="submitArea">
					<input type="submit" id="btn" onclick="confirmNum()" value="이메일 인증" />
				</div>
			<div id="innerbottom">
				<span>이메일을 받지 못하셨나요? 스팸함을 확인해보세요.<br /> <a href="">이메일을
						재발송</a>하거나 <a href="">이메일 주소를 변경</a>해 보세요.<br />
				</span>
			</div>
			<div id="innerbottom2">
				<span>이미 에픽게임즈 계정이 있나요?</span><a href="">로그인</a>
			</div>
		</div>
		<script>
			function confirmNum(){
				let sumNum = "";
				for(let i =1; i<=6; i++){
					sumNum += document.getElementById(i).value;
				}
 				if(sumNum == <%=confirmNum%>){
					location.href = "Controller?command=UpdateUserInfoAction&userName=<%=name%>&userPw=<%=pw%>&userEmail=<%=email%>&userNickname=<%=nickname%>";
				}else{
					alert("인증번호가 잘못되었습니다. 다시입력해주세요.");
				}
			}
		</script>
	</main>
